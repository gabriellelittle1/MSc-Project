...
class Room: 
    ...
    def draw(self):
        ...
        if self.fixed_objects:
            for obj in self.fixed_objects:
                rect = patches.Rectangle(obj.position[:2] - np.array([obj.width/2, obj.length/2]), obj.width, obj.length, linewidth=5, edgecolor='r', facecolor='none')
                ax.add_patch(rect)
                ax.text(obj.position[0], obj.position[1], obj.name, fontsize=10)
    ...
    #     ax.set_xlim(-2, self.width + 2)
        ...
        # color_dict = {}  # Dictionary to map object names to colors
        ...
        #         color_dict[obj.name] = color  # Store the color in the dictionary

        #         rectangle = patches.Rectangle(obj.position[:2] - np.array([obj.width / 2, obj.length / 2]), obj.width, obj.length, linewidth=2, edgecolor=color, facecolor='none', angle=np.rad2deg(obj.position[2]), rotation_point='center')
        #         ax.add_patch(rectangle)
        #         corners = obj.back_corners()
        #         for corner in corners:
        #             ax.plot(corner[0], corner[1], color=color, marker='o')

        # if self.fixed_objects:
        #     for obj in self.fixed_objects:
        #         color = 'r'  # Fixed objects have a fixed color
        #         rect = patches.Rectangle(obj.position[:2] - np.array([obj.width / 2, obj.length / 2]), obj.width, obj.length, linewidth=5, edgecolor=color, facecolor='none')
        #         ax.add_patch(rect)
        #         ax.text(obj.position[0], obj.position[1], obj.name, fontsize=10)

        # # Create legend
        # legend_patches = [patches.Patch(color=color, label=name) for name, color in color_dict.items()]
